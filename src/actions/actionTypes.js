// User
export const USER_SIGN_OUT_SUCCESS = 'USER_SIGN_OUT_SUCCESS';
export const USER_SIGN_OUT_FAIL = 'USER_SIGN_OUT_FAIL';
export const AUTH_FAIL = 'AUTH_FAIL';
export const AUTH_SUCCESS = 'AUTH_SUCCESS';
export const AUTH_START = 'AUTH_START';
export const SIGNIN_SIGNUP_FAIL = 'SIGNIN_SIGNUP_FAIL';
export const SIGNIN_SIGNUP_SUCCESS = 'SIGNIN_SIGNUP_SUCCESS';
export const SIGNIN_SIGNUP_START = 'SIGNIN_SIGNUP_START';

// Cart
export const ADD_ITEM_TO_CART = 'ADD_ITEM_TO_CART';
export const SHOW_CART_DROPDOWN = 'SHOW_CART_DROPDOWN';
export const REMOVE_ITEM_FROM_CART = 'REMOVE_ITEM_FROM_CART';
export const DECREASE_FROM_CART = 'DECREASE_FROM_CART';

// Shop
export const FETCH_COLLECTIONS_START = 'FETCH_COLLECTIONS_START';
export const FETCH_COLLECTIONS_FAIL = 'FETCH_COLLECTIONS_FAIL';
export const FETCH_COLLECTIONS_SUCCESS = 'FETCH_COLLECTIONS_SUCCESS';
